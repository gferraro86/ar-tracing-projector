...
export async function anchorOverlay(hitTestResult, frame, cornerPoints, centerPoint, centerQuat, referenceSpace, texture) {
  ...
  // Clear any existing overlay mesh (e.g. from a previous realign)
...
/**
 * Used during realign: keep the current overlay mesh visible (so the user
 * can see where it was before re-tapping) but release the anchor so a new
 * one can be created.
 */
